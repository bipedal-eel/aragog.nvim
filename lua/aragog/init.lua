...
local function set_autocmds()
	...
	vim.api.nvim_create_autocmd("BufLeave", {
		...
		callback = function(args)
			if
				not M.colony.current_burrow
				or not M.colony.current_thread
				or M.colony.current_thread.buf ~= args.buf
			then
				return
			end
...
function M.thread(idx)
	local ok, thread = pcall(function()
		return M.colony.current_burrow.threads[idx]
	end)
...
function M.burrow(idx)
	local ok, burrow = pcall(function()
		return M.colony.burrows and M.colony.burrows[idx]
	end)
...
function M.workspace(idx)
	local ws = M.ui.workspaces[idx]
	if not ws then return end
	change_dir_by_burrow({ dir = ws.path })
end
